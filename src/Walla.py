    ...
    for directory in directories:
        for filename in (join(directory, f) for f in listdir(directory)):
            if isfile(filename):
                ...
                warn(
                    "`%s' non è un file. `%s' dovrebbe avere solo immagini.",
                    (filename, directory),
                )
...
def walla_file(filename, options):
    ...
    if options.phase_1:
        output_file = '%s/%sx%s%s - %s' % (
            (base,)
            + size
            + (
                ' - %s' % split(base)[1] if options.add_dirname else '',
                nome_file,
            )
        )
...
def unwalla_file(input_file_name, options):
    ...
    output_file_name = join(
        root_input_file_name, ' - '.join(nome_file.split(' - ')[1:])
    )
...
def set_output_dir(value):
    ...
    with open(argv[0]) as infile:
        temp = (
            'OUTDIR = %r\n' % (value,) if line.startswith('OUTDIR') else line
            for line in infile
        )
...
def main():
    """Main function"""
    parser = OptionParser(
        version='%prog 0.5', usage="%prog [options] [DIRS='.'" '|FILES]'
    )
    parser.add_option(
        '-l',
        '--link',
        ...
        default=False,
        dest='link',
        callback=check_is_false('copy'),
        help='crea anche dei ' 'link simbolici nelle posizioni originarie',
    )
    parser.add_option(
        '-t',
        '--test',
    ...
    parser.add_option(
        ...
        default=False,
        dest='add_dirname',
        callback=check_is_false('phase_2'),
    ...
    parser.add_option(
        ...
        dest='OUTDIR',
        type=str,
        help='imposta una nuova directory di '
    ...
    parser.add_option(
        ...
        default=False,
        dest='phase_2',
        callback=check_is_false('phase_1', 'add_dirname'),
    ...
    parser.add_option(
        ...
        '--known-resolutions',
        action='callback',
        callback=known_resolutions,
        help='Mostra le risoluzioni accettate',
    )
