...
def mkparser():
    ...
    ret.add_option('-d', '--dir', action="callback", type='string', default='.',
            callback=check_dir, help="Sort files in DIR (default='%default')")
    ret.add_option('-b', '--begin', type="int", default=1, metavar='N',
            help="Start enumerating from N (default=%default)")
    ret.add_option('-v', '--verbose', action='store_true', default=False,
            help="Show on STDOUT what happens")
    ret.add_option('-c', '--cifre', type="int", default=0, metavar='N',
            help="Use at least N chars to represents the numbers")
    ret.add_option('-u', '--use-dirname', action='store_true', default=False,
            help="Use the dir name instead of the original name file")
    ret.add_option('-i', '--imgs', action='store_true', default=False,
            help="Files are images: sort then by pixel resolution, not by name")
    ret.add_option('-t', '--test', action='store_true', default=False,
            help="Test only: doesn't actually rename anything")
    ret.add_option('-a', '--also-dir', action='store_true', default=False,
...
def main():
    ...
    chdir(options.dir)
    filenames = sorted((filename for filename in listdir('.') if
            isfile(filename) or options.also_dir and isdir(filename)),
            key=sort_images if options.imgs else None, reverse=options.imgs)
        ...
        if not exists(destinazione):
            if options.verbose:
                ...
                if options.imgs:
                    ...
                    print "(%dx%d, %d, %d)" % (im_x, im_y, im_x*im_y,
                            -abs(im_x-im_y))
                else:
                    print
            if not options.test:
